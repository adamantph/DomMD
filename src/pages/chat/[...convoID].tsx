...
const Chat = () => {
	...
	//Fetch conversation if a convoID is given
	useEffect(() => {
		const getConversation = async () => {
                ...
                console.log(JSON.parse(result.conversation.messages))
				let fetchedMessages = JSON.parse(result.conversation.messages)
				let extractedData = fetchedMessages.map((message : any) => ({
					sender: message.from,
					message: message.content
				}));
				setMessages(extractedData)
				setConversationID((convoID ? convoID : '').toString())
        }
        getConversation();
	}, [convoID]);
